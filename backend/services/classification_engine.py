...
def run_classification_pipeline(
    ...
    """
    full_text = f"{email.subject or ''} {email.body_text or ''}".strip()
    if not full_text:
        logger.warning("Email %d has no subject or body — assigning default risk.", email.id)
    ...
    try:
        ...
        try:
            ...
            logger.error("Keyword matching failed for email %d: %s", email.id, e)
            keyword_score, top_rule = 0.0, None

        # ── Step 2: Sender Reputation ─────────────────────────────────────
        try:
            domain = email.sender_domain or (
                email.sender_email.split("@")[-1] if "@" in (email.sender_email or "") else "unknown"
            )
            ...
            logger.error("Reputation check failed for email %d: %s", email.id, e)
            reputation_risk = 50.0  # neutral fallback
        ...
        try:
            ...
            logger.error("Scan service failed for email %d: %s", email.id, e)
            link_records, att_records = [], []
        ...
        scan_score = min(
            (suspicious_links / total_links) * 100 + (50 if has_malicious_att else 0), 100.0
        )
        ...
        try:
            ...
            logger.error("ML model failed for email %d: %s", email.id, e)
            ml_spam_prob = 0.5  # neutral fallback

        # ── Step 5–9: Risk score, category, persist ────────────────────────
        ...
        try:
            from services.keyword_service import flush_keyword_hits
            flush_keyword_hits(db, kw_result.get("matched_ids", []))
            db.commit()
        except Exception as e:
            logger.error("Flush keyword hits failed for email %d: %s", email.id, e)
        ...
        if category in ("phishing", "unwanted") or risk_score >= 75:
            ...
            alert = Alert(
                ...
                description=(
                    ...
                    f"classified as {category.upper()} (risk={risk_score:.1f}/100)."
                ),
            )
            db.add(alert)
            db.commit()
        ...
        db.rollback()
        # Mark email as failed so it doesn't stay in limbo
        try:
            ...
            pass
        logger.exception("Classification pipeline failed for email %d: %s", email.id, e)
        raise
